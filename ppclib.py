...
#############################################
# Fetch PPC data files of daysback historical
""" import pandas as pd
import os
from datetime import datetime, timedelta

def fetch_ppc_days_back_org(start_date_str, daysback, output_dir='D:/api_ppc_saw/myfolder/'):  # ADDED output_dir PARAM
...
def fetch_ppc_days_back(start_date_str, daysback, output_dir='D:/api_ppc_saw/myfolder/'):
    # Set initial date and convert to datetime object
    start_date = datetime.strptime(start_date_str, "%Y-%m-%d")
    ...
    for i in range(daysback):
        ...
        # Special case: don't skip if it's the start date AND start date is today
        is_start_date = (i == 0)
        is_today = (current_date.date() == today_date)
        
        # Skip only if file exists AND it's not (start date AND today)
        if os.path.exists(output_file) and not (is_start_date and is_today):
            print(f"Skipping {current_date_str} (file already exists)")
            continue
...
#############################################
# Get parameter dictionaly from 'parameter' column
def get_parameter(s):
    pairs = s.split(',')
    parameter = {}
    for pair in pairs:
...
def plot_equipid_violin(anomalies_df):
    ...
    plt.tight_layout()
    plt.savefig('Distribution_by_Equipment_ID.png', dpi=300, bbox_inches='tight')  # Changed filename
    #plt.show()
...
def plot_trend(anomalies_df):
    ...
    plt.tight_layout()
    plt.savefig('Normalized_Anomaly_Score_Trend.png', dpi=300, bbox_inches='tight')  # Changed filename
    #plt.show()

#############################################
# Sort data and calculate moving average SVID_1404 (Chip Size Z1)
def plot_trend_chipZ1(anomalies_df):
    df = anomalies_df.sort_values('CreateTime')
    window_size = 30  # Adjust this based on your data frequency
    df['ChipSizeZ1_smoothed'] = df['SVID_1404'].rolling(
        window=window_size,
        min_periods=1,
        center=True
    ).mean()
    ...
    sns.lineplot(
        data=df,
        x='CreateTime',
        y='ChipSizeZ1_smoothed',
        color='#2c7bb6',
        linewidth=1.5,
        label=f'Smoothed ({window_size}-period MA)'
    )
    
    plt.title('Normalized ChipSizeZ1 Trend with Noise Reduction', fontsize=14, pad=20)
    plt.xlabel('Create Time', fontsize=12)
    plt.ylabel('Score Value', fontsize=12)
    plt.grid(True, alpha=0.3)
    plt.legend(loc='upper left')
    ...
    plt.tight_layout()
    plt.savefig('Normalized_ChipSizeZ1_Trend.png', dpi=300, bbox_inches='tight')  # Changed filename
    #plt.show()

#############################################
# Sort data and calculate moving average SVID_1404 (Chip Size Z1)
def plot_trend_chipZ2(anomalies_df):
    df = anomalies_df.sort_values('CreateTime')
    window_size = 30  # Adjust this based on your data frequency
    df['ChipSizeZ2_smoothed'] = df['SVID_1405'].rolling(
        window=window_size,
        min_periods=1,
        center=True
    ).mean()
    ...
    plt.tight_layout()
    plt.savefig('Normalized_ChipSizeZ2_Trend.png', dpi=300, bbox_inches='tight')  # Changed filename
    #plt.show()
...
def plot_violin_catagories(anomalies_df,cols_to_analyze):
    ...
    plt.tight_layout()
    plt.savefig('Distribution_by_Categories.png', dpi=300, bbox_inches='tight')  # Changed filename
    #plt.show()
...
def plot_mean_bars_catagoies(anomalies_df,cols_to_analyze):
    ...
    for i, col in enumerate(cols_to_analyze, 1):
        ...
        plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('Mean_Scores_by_Categories.png', dpi=300, bbox_inches='tight')  # Changed filename
    #plt.show()
...
def plot_heatmap(anomalies_df):
    ...
    plt.tight_layout()
    plt.savefig('Interaction_Heatmap.png', dpi=300, bbox_inches='tight')  # Changed filename
    #plt.show()
